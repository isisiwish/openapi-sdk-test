...
public class ReporterListenerImpl implements IReporter
{
    ...
    @Conf("klov.server")
    private static String klovServer;
    ...
    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory)
    {
        ...
        List<ExtentReporter> startedReporters = extent.getStartedReporters();
        for (ExtentReporter reporter : startedReporters)
        {
            if (reporter instanceof com.aventstack.extentreports.reporter.ExtentHtmlReporter)
            {
                try
                {
                    String filePath = ((ExtentHtmlReporter)reporter).getFilePath();
                    Field destination = reporter.getClass().getSuperclass().getDeclaredField("destination");
                    destination.setAccessible(true);
                    String rootPath = (String)destination.get(reporter);
                    String[] split = filePath.split("/");
                    String reportFile = rootPath + split[split.length - 1];
                    // todo upload to nginx export url
                    log.info("本次测试报告位于: {}", reportFile);
                }
                catch (Exception e)
                {
                    log.error("", e);
    ...
    private void init(List<XmlSuite> xmlSuites)
    {
        ...
        extent.setSystemInfo("Run path", System.getProperty("user.dir"));
        extent.setSystemInfo("Encoding", System.getProperty("file.encoding"));
        extent.setSystemInfo("url", "<a href='https://cfish.top'>cfish.top</a>");
        
        ExtentKlovReporter klov = new ExtentKlovReporter(projectName, reportTitle);
        klov.initMongoDbConnection(mongodbIp, Integer.valueOf(mongodbPort));
        ...
        if (tests.size() > 0)
        {
            ...
            for (ITestResult result : treeSet)
            {
                ...
                else
                {
                    ...
                    name = instanceName.substring(instanceName.lastIndexOf(".") + 1) + "." + result.getMethod().getMethodName() + "()";
                }
                if (extenttest == null)
                {
